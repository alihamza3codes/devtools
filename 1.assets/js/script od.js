...
function updateEditorHistory() {
    ...
    editorConfig.history.forEach(function(item, i){
        if(editorConfig.history.length > editorConfig.currentIndex){
            editorConfig.history.pop();
        }
    });


    let json = editor.toJSON();
    editorConfig.history.push(json);
    editorConfig.currentIndex = editorConfig.history.length - 1;
    if (editorConfig.history.length > 0) {
        enableBtn(undoButton);
    }
    l("Automatic Update : " + editorConfig.currentIndex);
}
editor.on({
    "object:modified": updateEditorHistory,
    "object:added": updateEditorHistory,
    "object:removed": updateEditorState
});
// Undo
function updateEditorState(val) {
    let stateNo = (editorConfig.currentIndex + val),
        currentHistory = editorConfig.history[stateNo];
    if (typeof currentHistory == 'undefined') return;

    editor.clear();
    editor.loadFromJSON(currentHistory, function() {
        editor.requestRenderAll();
        editorConfig.history.splice([editorConfig.history.length - 1], 1);
        editorConfig.currentIndex = stateNo;
        l("Undo history : " + editorConfig.currentIndex);
        l(editorConfig.history);
    });
}
